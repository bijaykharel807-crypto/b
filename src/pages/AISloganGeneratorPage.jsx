...
import { SloganStyle } from '../types.js';
import { generateSlogan } from '../services/geminiService.js';
 ...
 */
export const AISloganGeneratorPage = ({ onNavigate }) => {
  ...
  /** @type {React.useState<typeof SloganStyle[number]>} */
  const [selectedSloganStyle, setSelectedSloganStyle] = useState('catchy');
  /** @type {React.useState<boolean>} */
  ...
  const handleGenerateSlogan = useCallback(async () => {
    ...
    try {
      const newSlogan = await generateSlogan({
        ...
        style: selectedSloganStyle
      });

      setGeneratedSlogan(newSlogan);
      setIsTypewriting(true);
    } catch (error) {
      console.error('Failed to generate slogan:', error);
      setGeminiError(error.message || 'Failed to generate slogan.');
    } finally {
      setLoadingSlogan(false);
    }
  }, [selectedProduct, selectedSloganStyle]);

  return (
    <div className="container mx-auto px-4 py-16 min-h-screen">
      <h1 className="text-4xl font-bold text-center text-slate-800 mb-12">AI Product Slogan Generator</h1>
      ...
      <div className="max-w-3xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
        ...
        <div className="mb-8">
          ...
          <select
            ...
            aria-label="Select a product for slogan generation"
          >
        ...
        {selectedProduct && (
          <div className="mb-10 p-6 bg-slate-50 rounded-lg border border-slate-100 text-center">
            <h3 className="text-2xl font-bold text-slate-800 mb-3">{selectedProduct.name}</h3>
            <p className="text-xl text-indigo-700 font-semibold leading-relaxed italic min-h-[50px]">
              {isTypewriting ? <TypewriterText text={generatedSlogan} /> : generatedSlogan}
            </p>
          </div>
        )}
        ...
        <div className="relative group p-[2px] rounded-2xl overflow-hidden bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 animate-gradient-xy">
          <div className="bg-white rounded-[14px] p-8 shadow-inner relative z-10">
            <div className="flex items-center space-x-3 mb-6">
              ...
              <h3 className="text-xl font-bold text-slate-800">Generated AI Slogan</h3>
            </div>

            <div className="flex flex-wrap gap-2 mb-6" role="radiogroup" aria-label="Slogan style selection">
              {SloganStyle.map((style) => (
                <button
                  key={style}
                  onClick={() => setSelectedSloganStyle(style)}
                  className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${
                    selectedSloganStyle === style
                    ? 'bg-slate-800 text-white shadow-md'
                    : 'bg-slate-50 text-slate-400 hover:bg-slate-100'
                  }`}
                  role="radio"
                  aria-checked={selectedSloganStyle === style}
                  aria-label={`${style} slogan style`}
                >
                  {style}
                </button>
              ))}
            ...
            <Button
              ...
              disabled={loadingSlogan || !selectedProduct}
              className="w-full py-4 relative overflow-hidden group/ai"
              aria-label="Generate slogan with Gemini AI"
            >
              <span className={loadingSlogan ? 'opacity-0' : 'opacity-100'}>
                ✨ Generate Slogan with Gemini
              </span>
              {loadingSlogan && (
                <div className="absolute inset-0 flex items-center justify-center bg-white/80">
                  ...
                  <span className="text-indigo-600 font-bold">Dreaming up slogan...</span>
                </div>
              )}
            </Button>
